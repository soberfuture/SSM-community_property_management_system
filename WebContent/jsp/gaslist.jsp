...
<body style="margin: 0;padding: 0;">
	...
	<table id="dg" title="天然气表管理" iconCls="icon-imeter" class="easyui-datagrid"
		style="width: 100%;" url="${pageContext.request.contextPath}/gas/findByPagination.action"
	...
	<script type="text/javascript">
		...
		function setHouse(){
			$("#position").combobox({
				  url:"${pageContext.request.contextPath}/house/findGasHouse.action",//获取数据
				  // 向服务器请求的模式 
				  method : "post",
				  editable:false,
				  valueField: 'position',
		...
		var url;
		function add(){
			$.post('${pageContext.request.contextPath}/house/findGasHouse.action',function(data){
				if(data==null||data.length==0){
					$.messager.alert('信息','没有要添加的天然气表','info');
				}else{
					$('#dlg').dialog('open').dialog('setTitle','新增');
					$('#fm').form('clear');
					setHouse();
					url = '${pageContext.request.contextPath}/gas/insertGas.action';
				}
			});
		...
		function edit(){
			...
			if (row){
				...
				url = '${pageContext.request.contextPath}/gas/updateGas.action?gasId='+row.gasId;
			}else{
		...
		function editprice(){
			...
			if(price!=''){
				$.post('${pageContext.request.contextPath}/gas/editprice.action',{price:price},function(result){
					if (result.success){
						$('#dg').datagrid('reload');	// reload the user data
						$.messager.show({	// show error message
							title: '设置成功',
		...
		function remove(){
			...
			if (row){
				$.messager.confirm('删除','您确定要删除这一条记录吗?',function(r){
					if (r){
						$.post('${pageContext.request.contextPath}/gas/delGas.action',{gasId:row.gasId},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
								$.messager.show({	// show error message
									title: '成功',
		...
		function doSearch(value,name){
			var u;
			if(name == 'all'){
				u = "${pageContext.request.contextPath}/gas/findByPagination.action";
			}else{
				u = "${pageContext.request.contextPath}/gas/doSearch.action?name="+name+"&value="+value ;
			}
