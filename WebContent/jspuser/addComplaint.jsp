...
<body style="margin: 0;padding: 0;">
		<table id="dg" title="我的投诉" class="easyui-datagrid" style="width: 100%;background-color: #fff;"
	...
	<div id="toolbar">
		...
		<select id="complaintStatus" name="complaintStatus" class="easyui-combobox" panelHeight="auto">
	...
	<div id="dlg" class="easyui-dialog" style="width:350px;height:330px;padding:10px 20px"
	...
	<script type="text/javascript">
		...
		function add(){
			...
			url = '${pageContext.request.contextPath}/complaint/addComplaint.action';
		}
		
		function comment(){
			var row = $('#dg').datagrid('getSelected');
			if(row){
				if(row.complaintStatus=='已处理'){
					starInit();
					$('#comment').dialog('open').dialog('setTitle','评价');
					$('#cfm').form('clear');
					$('#eType').val('complaint');
					$('#targetId').val(row.complaintId);
		...
		$('#complaintStatus').combobox({
			onSelect:function(obj){
				var url = '${pageContext.request.contextPath}/complaint/findByPositionAndComplaintStatus.action';
